...
$taskBackend = Join-Path $taskRoot 'xonim_backend'
$taskFrontend = Join-Path $taskRoot 'xonim_frontend'
...
if (-not (Test-XonimPort -Port 8000)) {
    Start-XonimProcess -FilePath $taskPython -Arguments @('manage.py','runserver','127.0.0.1:8000','--noreload') -WorkingDirectory $taskBackend -PidPath (Join-Path $taskLogs 'backend.pid')
} else { Write-Output 'Port 8000 is occupied; existing process was not changed.' }
if (-not (Test-XonimPort -Port 5173)) {
    $taskNode = (Get-Command node.exe).Source
    Start-XonimProcess -FilePath $taskNode -Arguments @('node_modules/vite/bin/vite.js','--host','127.0.0.1') -WorkingDirectory $taskFrontend -PidPath (Join-Path $taskLogs 'frontend.pid')
