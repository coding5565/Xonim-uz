  ...
  return (
    <>
      <div className="page-heading">
        <div>
          <span className="eyebrow">MOLIYAVIY TARTIB</span>
          <h1>Xarajatlar<span className="heading-dot">.</span></h1>
          <p>Nimaga, kimga va qancha — har bir chiqim ochiq.</p>
        </div>
        <button className="button primary" onClick={start}><Plus size={18} />Xarajat kiritish</button>
      </div>
      <div className="expense-summary">
        <span className="metric-icon orange"><Wallet size={22} /></span>
        <div>
          <span className="muted">{narrowed ? 'Tanlangan kesim bo‘yicha' : 'Barcha kiritilgan xarajatlar'}</span>
          <h2>{money(sum)} <small>so‘m</small></h2>
        </div>
        <span className="pill">{inRange.length} ta yozuv</span>
      </div>
      ...
      <section className="panel report-filters">
        <header>
          <Filter size={19} />
          <div><h2>Davr va kategoriya</h2><p>Qaysi kundan qaysi kungacha, qaysi turdagi chiqim</p></div>
          {narrowed && (
      ...
      <section className="panel">
        <header className="panel-heading">
          <div><h2>Xarajatlar ro‘yxati</h2><p>Admin saqlashi bilan hisobga olinadi</p></div>
          <div className="search-field">
            <Search size={17} />
            <input
              value={query}
              onChange={event => setQuery(event.target.value)}
              placeholder="Maqsad, oluvchi, kategoriya…"
              aria-label="Xarajat qidirish"
            />
          </div>
        </header>
        <div className="table-wrap">
          <table>
            <thead>
              <tr><th>MAQSAD / OLUVCHI</th><th>KATEGORIYA</th><th>SANA</th><th>SUMMA</th><th>TO‘LOV</th><th>KIRITGAN</th></tr>
            </thead>
            <tbody>
              {filtered.map(row => (
                <tr key={row.id}>
                  <td><strong>{row.purpose}</strong><small>{row.recipient || 'Oluvchi ko‘rsatilmagan'}</small></td>
                  ...
                  <td>
                    <span className={`status ${row.payment_method === 'unpaid' ? 'open' : 'paid'}`}>
                      {row.payment_method === 'cash' ? 'Naqd' : row.payment_method === 'card' ? 'Karta' : 'To‘lanmagan'}
                    </span>
                  </td>
          ...
          {!filtered.length && (
            <div className="empty-state">
              ...
              <h3>Xarajatlar hali yo‘q</h3>
              <p>Ijara, kommunal va boshqa kundalik xarajatlarni kiriting.</p>
            </div>
          )}
        </div>
      </section>
      <p className="data-note">
        Bu bo‘lim kundalik xarajatlar uchun. Ombor xaridi qiymati va ikkiyoqlama buxgalteriya registri hali ulanmagan.
      </p>

      <AppModal open={open} title="Yangi xarajat" onClose={() => { if (!busy) setOpen(false) }}>
        <form onSubmit={save}>
            ...
            <div className="form-row">
              ...
              <label>
                ...
                <select value={form.payment_method} onChange={event => update({ payment_method: event.target.value })}>
                  <option value="cash">Naqd to‘landi</option>
                  <option value="card">Karta to‘landi</option>
                  <option value="unpaid">Hali to‘lanmagan</option>
                </select>
              </label>
            </div>
          </fieldset>
          <p className="alert">Saqlangach darhol hisobga olinadi. Superadmin tasdig‘i talab qilinmaydi.</p>
          {formError && <p className="alert error">{formError}</p>}
          <button className="button primary full" disabled={busy}>
            {busy ? 'Saqlanmoqda…' : submitted ? 'Oldingi amalni qayta tekshirish' : 'Xarajatni saqlash'}
          </button>
        </form>
      </AppModal>
    </>
